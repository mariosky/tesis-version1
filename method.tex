...
This chapter presents the proposed method in detail.
Data models are defined in order to explain the characteristics (or
information) used from each model. \\Overall, the post-filtering
approach is applied in order to cover the user's needs in a wide
range of satisfaction and, it is based in the information that users
provide in their profile. The chapter describes each
component in the method, as well as its functionality and interaction
with another components of the method.
...
The method is based  in a previous architecture presented in \cite{profe}
used in a e-learning domain where the context involves characteristics
referring the student enviroment as well as their knowledge about any
topic.  After that, another recommender system  called
\textbf{Recomet} \cite{recomet}  implements the method using only traditional
recommender systems,  i.e., does not apply the use of context to
suggest recommendations for users.  The propose method is based in the
post-filtering approach, this paradigm  contextualize the
recommendations list (see figure \ref{fig:paradigms}), previously three
recommendations techniques working simultaneously to provides the
recommendations list. Once in a time user defines the current context
the  post-filtering adjust the recommendations for that user context,
...
The recommendation techniques are the following:
\begin{enumerate} 
...
it considers the following variables: \textit{ratings average: low,medium and high},
